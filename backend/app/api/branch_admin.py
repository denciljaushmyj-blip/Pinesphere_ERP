...
def _admission_rows(db: Session, branch_id: str | None, branch: Branch | None, status: str | None = None):
    rows = [_lead_payload(db, lead, branch) for lead in _lead_query(db, branch_id).order_by(Lead.created_at.desc()).all()]
    if not status:
        return rows
...
def approve_admission(lead_id: str, payload: dict | None = None, branch_id: str | None = None, db: Session = Depends(get_db), current_user=Depends(require_roles(UserRole.BRANCH_ADMIN, UserRole.SUPER_ADMIN))):
    ...
    db.add(student)
    db.flush()
    add_history(db, module="Students", action="student_created", title=f"Student created: {student.full_name}", details=f"Admission: {lead.id} | Login email: {student.email} | Login password: Admin@123", record_id=student.id, created_by_id=current_user.id, branch_id=scoped_branch_id)
...
def students(branch_id: str | None = None, course_id: str | None = None, batch_name: str | None = None, status: str | None = None, db: Session = Depends(get_db), current_user=Depends(require_roles(UserRole.BRANCH_ADMIN, UserRole.SUPER_ADMIN))):
    ...
    return [_student_payload(db, student) for student in query.order_by(User.created_at.desc()).all()]
...
def create_student(payload: dict, branch_id: str | None = None, db: Session = Depends(get_db), current_user=Depends(require_roles(UserRole.BRANCH_ADMIN, UserRole.SUPER_ADMIN))):
    ...
    student = User(
        ...
        hashed_password=hash_password(payload.get("password") or "Admin@123"),
        role=UserRole.STUDENT,
        role_abbreviation=role_abbreviation(UserRole.STUDENT),
        branch_id=scoped_branch_id,
...
def get_branch_users(
    ...
    return [_user_detail_payload(db, u, branch) for u in users_list]
...
def create_branch_user(
    ...
    password = payload.get("password") or "Admin@123"
...
def fee_invoices(branch_id: str | None = None, db: Session = Depends(get_db), current_user=Depends(require_roles(UserRole.BRANCH_ADMIN, UserRole.SUPER_ADMIN))):
    scoped_branch_id, _ = _scope(db, current_user, branch_id)
    return [_invoice_payload(invoice) for invoice in _invoice_query(db, scoped_branch_id).order_by(Invoice.created_at.desc()).all()]
...
def fee_payments(branch_id: str | None = None, db: Session = Depends(get_db), current_user=Depends(require_roles(UserRole.BRANCH_ADMIN, UserRole.SUPER_ADMIN))):
    scoped_branch_id, _ = _scope(db, current_user, branch_id)
    return [_payment_payload(payment) for payment in _payment_query(db, scoped_branch_id).order_by(Payment.paid_at.desc()).all()]
...
def fee_receipts(branch_id: str | None = None, db: Session = Depends(get_db), current_user=Depends(require_roles(UserRole.BRANCH_ADMIN, UserRole.SUPER_ADMIN))):
    scoped_branch_id, _ = _scope(db, current_user, branch_id)
    return [_receipt_payload(payment) for payment in _payment_query(db, scoped_branch_id).order_by(Payment.paid_at.desc()).all()]
...
@router.get("/batches/{batch_name}/students")
def batch_students(batch_name: str, branch_id: str | None = None, db: Session = Depends(get_db), current_user=Depends(require_roles(UserRole.BRANCH_ADMIN, UserRole.SUPER_ADMIN))):
    scoped_branch_id, _ = _scope(db, current_user, branch_id)
    query = _student_query(db, scoped_branch_id).filter(User.batch_name == batch_name)
    enrollment_students = db.query(User).join(Enrollment, Enrollment.student_id == User.id).filter(Enrollment.batch_name == batch_name, User.role == UserRole.STUDENT)
    if scoped_branch_id:
