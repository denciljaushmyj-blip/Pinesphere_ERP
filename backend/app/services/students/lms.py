...
def get_student_course_with_materials(
    ...
    - 403 if course is not published (unpublished = trainer-only)
    ...
    # 5. For each lesson, fetch materials (lesson-specific + course-level)
    lesson_responses = []
    for lesson in lessons:
            ...
            .filter(
                and_(
                    ...
                    or_(
                        ...
                        TrainerLessonMaterial.lesson_id.is_(None),
                    ),
                )
            )
        ...
        material_responses = [_material_response(m) for m in materials]
