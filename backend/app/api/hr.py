...
def list_employees(
    ...
    return [_employee_response(row, branches, attendance) for row in query.order_by(Employee.full_name.asc()).all()]
...
def list_attendance(
    ...
    branches = _branch_map(db)
    return [_attendance_response(row, branches) for row in query.order_by(StaffAttendance.attendance_date.desc()).all()]
...
def list_payroll(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    if _is_branch_limited(current_user):
        query = query.filter(Employee.branch_id == current_user.branch_id)
    return [_payroll_response(row) for row in query.order_by(Payroll.year.desc(), Payroll.month.desc()).all()]
...
def list_trainer_workload(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    branches = _branch_map(db)
    return [_workload_response(row, branches) for row in query.order_by(TrainerWorkload.updated_at.desc()).all()]
...
def list_performance(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    if _is_branch_limited(current_user):
        query = query.filter(Employee.branch_id == current_user.branch_id)
    return [_performance_response(row) for row in query.order_by(PerformanceReview.created_at.desc()).all()]
...
def list_tasks(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    if _is_branch_limited(current_user):
        query = query.filter(Employee.branch_id == current_user.branch_id)
    return [_task_response(row) for row in query.order_by(StaffTask.created_at.desc()).all()]
