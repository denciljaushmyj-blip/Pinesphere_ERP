...
export function UserManagementPanel({ accessToken }: { accessToken: string }) {
  ...
  async function refreshUsers(extraHiddenUserIds: string[] = []) {
    ...
    try {
      const [userData, branchData] = await Promise.all([
        apiRequest<UserRow[]>("/auth/users", accessToken),
        /* =====================================================
  ...
  useEffect(() => {
    ...
    async function loadInitialUsers() {
      ...
      setError("");
      try {
        const [data, branchData, inviteData] = await Promise.all([
          apiRequest<UserRow[]>("/auth/users", accessToken),
          apiRequest<BranchResponse[]>("/branches", accessToken).catch(() => []),
  ...
  async function sendInvite(reactivateExisting = false) {
    ...
    try {
      ...
      setInviteForm({ email: "", role: "trainer", branch_id: "main", temporary_password: "", delivery: "auto" });
