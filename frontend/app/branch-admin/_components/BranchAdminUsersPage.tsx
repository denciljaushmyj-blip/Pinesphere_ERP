...
export function BranchAdminUsersPage() {
  ...
  const loadData = async () => {
    setLoading(true)
    try {
      const [allUsers, courseOpts, batchOpts, roleOpts] = await Promise.all([
        getBranchUsers(),
        getCourseOptions(),
        getBatchOptions(),
      ...
      const scopedUsers = allUsers.filter(u => !u.branch_id || u.branch_id === branch.branch_id)
  ...
  const batchNames = useMemo(() => Array.from(new Set(batches.map((item) => item.label).filter(Boolean))), [batches])
