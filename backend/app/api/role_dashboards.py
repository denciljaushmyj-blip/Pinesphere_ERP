...
def _student_payload(db: Session, current_user: User):
    ...
    for enrollment in enrollments:
        ...
        courses.append({
            ...
            "remainingLessons": max(0, 36 - round((enrollment.progress_percent / 100) * 36)),
    ...
    return {
        ...
        "assignmentsCompleted": db.query(Lesson).join(Lesson.progress_records).filter(Lesson.content_type == "assignment").count(),
        "enrolledCourses": courses,
        "assignmentsDue": [
            # TODO: Replace with assignments/submissions tables when the assignment module is migrated.
            {"id": "assignment-placeholder", "title": "No assignment table connected yet", "course": "LMS", "due": "Upcoming", "priority": "Low"}
        ],
        "certificates": certificates,
        "placementReadiness": {
            "resumeScore": 0,
