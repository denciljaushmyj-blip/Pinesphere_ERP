...
const moduleStyles: Record<ModuleLabel, { gradient: string; accent: string; soft: string; border: string; icon: string; shadow: string }> = {
  Dashboard: { gradient: duoGradients.pineHeader,  accent: duo.green,   soft: duo.greenSoft,  border: "#cceabf",  icon: "🏠", shadow: "rgba(47,125,0,0.22)" },
  Users:     { gradient: duoGradients.blue,   accent: duo.blue,    soft: duo.blueSoft,   border: duo.blueDark,   icon: "👥", shadow: "rgba(28,176,246,0.35)" },
  Branches:  { gradient: duoGradients.teal,   accent: duo.teal,    soft: duo.tealSoft,   border: duo.tealDark,   icon: "🏢", shadow: "rgba(0,205,156,0.35)" },
  CRM:       { gradient: duoGradients.pineHeader, accent: duo.green,  soft: duo.greenSoft, border: "#cceabf", icon: "📊", shadow: "rgba(47,125,0,0.22)" },
  Students:  { gradient: duoGradients.pineHeader, accent: duo.green,  soft: duo.greenSoft, border: "#cceabf", icon: "🎓", shadow: "rgba(47,125,0,0.22)" },
  LMS:       { gradient: duoGradients.pineHeader, accent: duo.green,  soft: duo.greenSoft, border: "#cceabf", icon: "📚", shadow: "rgba(47,125,0,0.22)" },
  Finance:   { gradient: duoGradients.pineHeader,  accent: duo.green,   soft: duo.greenSoft,  border: "#cceabf",  icon: "💰", shadow: "rgba(47,125,0,0.22)" },
  HR:        { gradient: duoGradients.blue,   accent: duo.blue,    soft: duo.blueSoft,   border: duo.blueDark,   icon: "👔", shadow: "rgba(28,176,246,0.35)" },
  AI:        { gradient: duoGradients.purple, accent: duo.purple,  soft: duo.purpleSoft, border: duo.purpleDark, icon: "🤖", shadow: "rgba(206,130,255,0.35)" },
  Franchise: { gradient: duoGradients.orange, accent: duo.orange,  soft: duo.orangeSoft, border: duo.orangeDark, icon: "🏪", shadow: "rgba(255,150,0,0.35)" },
  Reports:   { gradient: duoGradients.teal,   accent: duo.teal,    soft: duo.tealSoft,   border: duo.tealDark,   icon: "📈", shadow: "rgba(0,205,156,0.35)" },
  Security:  { gradient: duoGradients.pineHeader,    accent: duo.green,     soft: duo.greenSoft,    border: "#cceabf",    icon: "🔒", shadow: "rgba(47,125,0,0.22)" },
  Settings:  { gradient: duoGradients.pineHeader,   accent: duo.green,    soft: duo.greenSoft,   border: "#cceabf",   icon: "⚙️", shadow: "rgba(47,125,0,0.22)" },
...
function normalizeAuthProfile(profile: AuthProfile | null): AuthProfile | null {
...
function getLoginRedirectPath(profile: AuthProfile | null) {
  const role = normalizeUserRole(profile?.role) ?? normalizeUserRole(profile?.role_abbreviation);
...
const alertStyleBySeverity: Record<string, { accent: string; soft: string; border: string; emoji: string }> = {
  critical: { accent: duo.red,    soft: duo.redSoft,    border: duo.redDark,   emoji: "🚨" },
  warning:  { accent: duo.orange, soft: duo.orangeSoft, border: duo.orangeDark, emoji: "⚠️" },
  info:     { accent: duo.blue,   soft: duo.blueSoft,   border: duo.blueDark,  emoji: "ℹ️" },
  success:  { accent: duo.green,  soft: duo.greenSoft,  border: duo.greenDark, emoji: "✅" },
};
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        const profile = normalizeAuthProfile(await response.json() as AuthProfile);
        saveSession({ accessToken:token, refreshToken, profile, rememberMe });
        navigateTo(getLoginRedirectPath(profile));
      } catch {
  ...
  function saveSession(next: AuthSession) {
    const profile = normalizeAuthProfile(next.profile);
    const normalizedSession = { ...next, profile };
    clearStoredSession();
    if (normalizedSession.accessToken) {
      storeSessionValue("pinesphere_access_token", normalizedSession.accessToken, normalizedSession.rememberMe);
      storeSessionValue("pinesphere_refresh_token", normalizedSession.refreshToken, normalizedSession.rememberMe);
      storeSessionValue("pinesphere_profile", JSON.stringify(profile), normalizedSession.rememberMe);
    }
  ...
  async function handleLogout() {
    ...
    if (authSession.accessToken && authSession.refreshToken) {
      await fetch(`${API_URL}/auth/logout`, { method:"POST", headers:{ Authorization:`Bearer ${authSession.accessToken}`, "Content-Type":"application/json" }, body:JSON.stringify({ refresh_token:authSession.refreshToken }) }).catch(()=>undefined);
    }
    saveSession({ accessToken:"", refreshToken:"", profile:null, rememberMe:false });
  ...
  return (
    <main className={isDark ? "dark" : undefined} style={{ minHeight:"100vh", background: isDark ? duo.bgDark : duo.bgAlt, fontFamily:"'Nunito',sans-serif" }}>
      ...
      {loginOpen && (
        <LoginDialog onClose={() => setLoginOpen(false)} onLogin={(s) => { const profile = normalizeAuthProfile(s.profile); saveSession({ ...s, profile }); navigateTo(getLoginRedirectPath(profile)); setToast(`Welcome back, ${roleDisplayNames[profile?.role ?? ""] ?? "User"}.`); setLoginOpen(false); }} />
      )}
          ...
          {courses.map(course=>(
            <article className="public-card" key={course.title} style={{ ...glass, borderRadius:24, overflow:"hidden" }}>
              ...
              <div style={{ padding:20 }}>
                ...
                <p style={{ margin:"0 0 14px", color:"#687386", fontWeight:800 }}>{course.level} � {course.duration}</p>
    ...
    <div style={{ position:"fixed", inset:0, zIndex:70, display:"flex", alignItems:"center", justifyContent:"center", padding:18, background:"rgba(7,18,38,.58)", backdropFilter:"blur(10px)" }}>
      <div style={{ width:"100%", maxWidth:760, maxHeight:"92vh", overflowY:"auto", borderRadius:28, background:"linear-gradient(180deg,rgba(255,255,255,.96),rgba(246,250,255,.96))", border:"1px solid rgba(255,255,255,.75)", boxShadow:"0 30px 90px rgba(7,18,38,.28)", padding:26 }}>
        ...
        {step===4&&<div style={{ display:"grid", gap:16 }}>
          ...
          {done&&<div style={{ textAlign:"center", fontSize:42, animation:"publicFloat 1.8s ease-in-out infinite" }}>✅</div>}
              ...
              <button type="button" onClick={onOpenProfile} title="Edit profile" style={{ textAlign:"right", display:"none", border:"none", background:"transparent", cursor:"pointer", fontFamily:"'Nunito',sans-serif" }} className="md-block">
                ...
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:4, background:duo.yellowSoft, border:`2px solid ${duo.yellowDark}`, borderRadius:50, padding:"4px 12px" }}>
                    <Flame size={14} color={duo.orange}/><span style={{ fontSize:12, fontWeight:800, color:duo.orangeDark }}>47 🔥</span>
                  </div>
  ...
  return (
    <div style={{ height:"100%", display:"flex", flexDirection:"column", padding:18, overflow:"hidden" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:24, padding:"12px 14px", background:t.logoBg, borderRadius:14, border:`1px solid ${t.logoBorder}` }}>
        <div style={{ width:48, height:48, borderRadius:14, background:duoGradients.green, display:"flex", alignItems:"center", justifyContent:"center", fontSize:22, flexShrink:0, border:`2px solid ${duo.greenDark}`, boxShadow:"0 4px 0 rgba(70,163,2,0.4)" }}>🎓</div>
        <div>
      ...
      <div style={{ marginTop:16, padding:"13px 14px", background:t.badgeBg, border:`1px solid ${t.badgeBorder}`, borderRadius:12 }}>
        ...
        <p style={{ margin:"4px 0 0", fontSize:11, color:t.badgeSub, fontWeight:700 }}>✅ Smart alerts active</p>
      </div>
    </div>
  );
}
...
function DashboardContent({ accessToken,allowedModules,onRefreshSession,onNavigate,isDark=false }:{
  ...
  const total = dashboard ? `${dashboard.total_students_active+dashboard.total_students_inactive}` : "—";
  const rev   = dashboard ? formatCurrency(dashboard.revenue_this_month) : "—";
  const att   = dashboard ? `${Math.round(dashboard.attendance_rate_today)}%` : "—";
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:24 }}>
      ...
      <div style={{ borderRadius:duo.radiusXl, background:duoGradients.pineHeader, padding:"26px 32px", position:"relative", overflow:"hidden", border:"3px solid #2f7d00", boxShadow:"0 7px 0 rgba(47,125,0,0.35)" }}>
        ...
        {/* Decorative emoji */}
        <div style={{ position:"absolute", top:16, right:180, fontSize:40, opacity:0.2, pointerEvents:"none" }}>🌟</div>
        <div style={{ position:"absolute", bottom:10, right:60, fontSize:32, opacity:0.15, pointerEvents:"none" }}>🎯</div>
        <div style={{ position:"relative", display:"flex", flexWrap:"wrap", gap:20, alignItems:"center" }}>
          <div style={{ flex:1, minWidth:220 }}>
            ...
            {status && <div style={{ marginTop:10, display:"inline-flex", alignItems:"center", gap:6, background:"rgba(255,255,255,0.2)", borderRadius:50, padding:"6px 16px" }}><span style={{ fontSize:11, color:"#fff", fontWeight:700 }}>⏳ {status}</span></div>}
          </div>
          {[["🎓","Total Students",total,"Students"],[" 💰","Revenue",rev,"Finance"],["✅","Attendance",att,"HR"]].map(([icon,lbl,val,mod])=>(
      ...
      {/* AI Insights */}
      <GlassPanel title="🧠 AI Insights" subtitle="Duolingo-style motivation, but focused on institute operations" isDark={isDark}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(320px,1fr))", gap:20 }}>
        <GlassPanel title="🏢 Branch Performance" subtitle="Students, attendance, revenue by branch" isDark={isDark}>
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            {liveBranches.map(b=><BranchRow key={b.name} branch={b} isDark={isDark}/>)}
          </div>
        </GlassPanel>
        <GlassPanel title="🤖 AI Alerts" subtitle="Smart operating alerts" isDark={isDark}>
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
...
function ModuleContent({ accessToken, module, profile, onRefreshSession, isDark=false }:{ accessToken:string; module:ModuleLabel; profile:AuthProfile; onRefreshSession:()=>Promise<string|undefined>; isDark?:boolean }) {
  ...
  useEffect(()=>{
    ...
    async function load() {
      ...
      try {
        ...
        setNotice(`${moduleCopy[module].title} ready ✅`);
  ...
  async function deleteRecord(){
    ...
    try{
      ...
      setNotice(`${del.title} deleted ✅`);setConfirmDelete(null);
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))", gap:12 }}>
        {[["📊 Active records",loading?"⏳":`${activeRecords}`,"Current",""],["🔔 Pending actions",loading?"⏳":`${pendingRecords}`,"Needs review",""],["💚 Health score",activeRecords?"100%":"Ready","System OK",""]].map(([lbl,val,helper])=>(
      ...
      {showModuleSearch && (
        <GlassPanel title={`🔎 ${module} search`} subtitle="Search inside this module by name, ID, phone, status, course or record details." isDark={isDark}>
          <div style={{ position:"relative" }}>
      ...
      {module==="Students" && searchedStudents.length>0 && !isSuperAdmin && (
        <GlassPanel title="🎓 Student lifecycle" subtitle="Admission, documents, batch, course readiness" isDark={isDark}>
      ...
      {module==="CRM" && !isSuperAdmin && (
        <GlassPanel title="📊 CRM pipeline" subtitle="Lead source, follow-up, demo status, conversion" isDark={isDark}>
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
      ...
      {module==="LMS" && !isSuperAdmin && (
        <GlassPanel title="📚 LMS delivery" subtitle="Course publishing, batch assignment, learner progress" isDark={isDark}>
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
      ...
      {module==="Finance" && !isSuperAdmin && (
        <>
          <GlassPanel title="💰 Fee collection" subtitle="Invoices, payment status, dues, collection progress" isDark={isDark}>
            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
      ...
      {module==="Security" && (
        <>
          <GlassPanel title="🔒 Security dashboard" subtitle="Audit trails, sessions, failed logins" isDark={isDark}>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))", gap:12 }}>
              {[["📋 Audit logs",snapshot.securitySummary?.audit_logs??0],["🟢 Active sessions",snapshot.securitySummary?.active_sessions??0],["🚫 Revoked",snapshot.securitySummary?.revoked_sessions??0],["❌ Failed logins",snapshot.securitySummary?.failed_logins??0],["⚠️ Events",snapshot.securitySummary?.suspicious_events??0]].map(([lbl,val])=>(
          ...
          <GlassPanel title="📋 Recent audit logs" subtitle="Sensitive platform actions" isDark={isDark}>
...
{module==="CRM" && (
        <GlassPanel title="📋 CRM leads" subtitle="Latest leads and admission enquiries" isDark={isDark}>
      ...
      {((module==="Students"&&!isStudentLogin&&!isSuperAdmin)||module==="Security") && (
        <GlassPanel title="👥 Users" subtitle="Student and staff account records" isDark={isDark}>
      ...
      {(module==="LMS"||module==="Students") && (
        <GlassPanel title="📖 Courses" subtitle="Available courses and batches" isDark={isDark}>
      ...
      {module==="LMS" && (
        <GlassPanel title="📅 Attendance sessions" subtitle="Upcoming and recorded class sessions" isDark={isDark}>
  ...
  return (
    <button onClick={onClick} style={{ background: isDark?"rgba(255,255,255,0.05)":"#FFFFFF", border:`2px solid ${isDark?"rgba(255,255,255,0.12)":duo.border}`, borderRadius:16, padding:"15px", textAlign:"left", cursor:"pointer", width:"100%", minHeight:154, position:"relative", overflow:"hidden", transition:"transform 0.15s, box-shadow 0.15s", boxShadow: isDark?"none":"0 4px 0 rgba(0,0,0,0.06)", fontFamily:"'Nunito',sans-serif" }}
      ...
      <div style={{ display:"inline-flex", alignItems:"center", gap:4, background: isDark?"rgba(88,204,2,0.15)":duo.greenSoft, border:`1px solid ${duo.greenDark}`, borderRadius:50, padding:"3px 12px" }}>
        <span style={{ fontSize:11, fontWeight:800, color: isDark?"#58CC02":duo.greenDark }}>✅ {metric.trend}</span>
      </div>
    </button>
  );
}
  ...
  return (
    <div style={{ background: isDark?"rgba(255,255,255,0.04)":"#F7FBF0", border:`2px solid ${isDark?"rgba(255,255,255,0.08)":duo.border}`, borderRadius:duo.radius, padding:"14px 16px", boxShadow: isDark?"none":"0 3px 0 rgba(0,0,0,0.05)" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
        <p style={{ margin:0, fontWeight:800, color: isDark?"#fff":duo.text, fontSize:14 }}>🏢 {branch.name}</p>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:8, marginBottom:10 }}>
        {[`👨‍🎓 ${branch.students}`,`✅ ${branch.attendance}%`,`🎯 ${branch.conversion}%`].map(t=>(
            ...
            <tr key={row.key} style={{ background:i%2===0?(isDark?"rgba(255,255,255,0.02)":"#FFFFFF"):(isDark?"rgba(88,204,2,0.04)":"#FBFFF7"), borderBottom:`1px solid ${border}` }}>
              ...
              <td style={{ padding:"10px 12px", textAlign:"center", whiteSpace:"nowrap" }}>
                <div style={{ display:"inline-flex", gap:6 }}>
                  <IconAction label="View" symbol="👁" onClick={row.onView}/>
                  <IconAction label="Edit" symbol="✎" onClick={row.onEdit}/>
                  <IconAction label="Disable" symbol="⏻" onClick={row.onDisable}/>
                  <IconAction label="Delete" symbol="🗑" danger onClick={row.onDelete}/>
                </div>
              </td>
            </tr>
          ))}
    ...
    <div style={{ background: isDark?"rgba(255,255,255,0.04)":"#FFFFFF", border:`2px solid ${isDark?"rgba(255,255,255,0.08)":duo.border}`, borderRadius:duo.radius, padding:"14px 16px", boxShadow: isDark?"none":"0 3px 0 rgba(0,0,0,0.05)" }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:12, marginBottom:10 }}>
        ...
        <div style={{ display:"flex", gap:8, flexShrink:0 }}>
          ...
          {onDelete && <button onClick={onDelete} style={{ fontSize:12, fontWeight:800, padding:"4px 14px", borderRadius:50, background:duo.redSoft, border:`2px solid ${duo.redDark}`, color:duo.red, cursor:"pointer", fontFamily:"'Nunito',sans-serif" }}>🗑️ Delete</button>}
        </div>
      </div>
...
function ActionModal({ accessToken,action,courses=[],students=[],leads=[],invoices=[],onClose,onComplete }:{
  ...
  async function runAction(){
    ...
    setBusy(true);setResponseText("Sending... 🚀");setHistoryRows(null);
    try{
...
function HistoryTable({ rows }:{ rows:HistoryEventRow[] }) {
  if(!rows.length) return <div style={{ padding:"24px", borderRadius:duo.radiusSm, border:`2px dashed ${duo.border}`, textAlign:"center", fontSize:13, color:duo.textMuted, fontWeight:700 }}>🌱 No history found yet.</div>;
  return (
